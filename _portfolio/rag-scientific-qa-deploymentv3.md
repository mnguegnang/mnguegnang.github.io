...
excerpt: "Production-ready RAG system for scientific QA over NLP papers — hybrid FAISS + BM25 retrieval, ColBERT v2 reranking, full CRAG framework, and GitHub Actions CI/CD publishing immutable Docker images to GHCR.<br/><a href='https://github.com/mnguegnang/RAG-app-scientific-QA' class='btn btn--info'>Deployment Repo</a> <a href='https://github.com/mnguegnang/RAG-for-Scientific-QA' class='btn btn--info'>Research Repo</a>"
collection: portfolio
---

## TL;DR

- **Situation:** Researchers navigating thousands of NLP papers waste time manually synthesising answers from keyword search results that return documents, not answers.
- **Action:** Designed and deployed a production RAG application — FastAPI backend, Chainlit chat UI, hybrid FAISS + BM25 retrieval, ColBERT v2 late-interaction reranking, full CRAG framework generation, and a GitHub Actions CI/CD pipeline publishing immutable Docker images to GHCR on every commit.
- **Result:** One-command deployment (`docker compose up`) on CPU or GPU hardware, with real-time token-streamed cited answers, transparent chain-of-thought reasoning, and hallucination suppression via the full CRAG framework. Evaluated on **84 QASPER questions** — ALCE Citation Precision **0.8373** (ongoing — additional metrics to be published).
...
Grounded cited answers delivered with visible reasoning, hallucinations suppressed by the full CRAG framework, and the full stack deployable on commodity hardware via a single command.
...
**Stage 3 — Full CRAG Framework.** Implements the complete Corrective RAG framework with three actions: **Correct** (retrieved context is confidently relevant — proceed to generation), **Incorrect** (retrieved context is below threshold — suppress generation and return a low-confidence warning), and **Ambiguous** (borderline confidence — generation proceeds with a hedged response). This replaces the earlier binary gate with a more nuanced relevance policy.
...
**Why ColBERT v2 over a cross-encoder reranker?** Cross-encoders score each query-document pair jointly, which is accurate but requires a full forward pass per candidate at query time. ColBERT v2 pre-computes document token embeddings offline; query time only needs a MaxSim operation — scoring each query token against all document token embeddings and summing the maximums. This delivers comparable or better precision at a fraction of the latency, making it more suitable for production serving.
...
The RAG pipeline was evaluated on **84 QASPER questions** using ALCE (citation grounding via NLI), with the upgraded ColBERT v2 reranker and full CRAG framework. Additional metrics will be published in a future evaluation run.
